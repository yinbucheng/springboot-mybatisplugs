...
public class MyWatch implements Watcher {
    ...
    public ZooKeeper createZkConnection(String address, int timeout) {
    ...
    public static void getChild(ZooKeeper zk, String path) throws Exception {
        zk.getChildren(path, (watchedEvent -> {
            String tempPath = watchedEvent.getPath();
            EventType type = watchedEvent.getType();
            if (type == EventType.NodeDeleted) {
                System.out.println("Delete:" + tempPath);
            } else if (type == EventType.NodeCreated) {
                System.out.println("Create:" + tempPath);
            }
            System.out.println(tempPath+" "+type);
            try {
                getChild(zk, path);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static void main(String[] args) throws Exception {
        ...
        ZooKeeper zk = w.createZkConnection(ADDRESS, TIMEOUT);
        getChild(zk, "/");
        zk.create("/intellif-test", "".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        zk.create("/intellif-test/test", "".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
        zk.create("/intellif-test/test2", "".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
        zk.delete("/intellif-test/test", -1);
        zk.create("/intellif-test/test3", "".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
    }
